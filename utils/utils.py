...
def per_user_ndcg_hr(data_rank, topk=10):
    """Return per-user NDCG@topk and HR@topk as numpy arrays (shape: [N_users]).

    Unlike metric_report() which returns aggregate means, this function keeps
    one score per user so that downstream paired t-tests have N_users data
    points instead of just N_seeds.

    Args:
        data_rank: array-like of shape [N_users], each entry is the 0-indexed
                   rank of the target item in the ranked candidate list.
        topk:      cut-off (default 10).

    Returns:
        ndcg : np.ndarray [N_users], NDCG@topk per user (0 if not in top-k).
        hr   : np.ndarray [N_users], HR@topk per user  (0 or 1).
    """
    data_rank = np.asarray(data_rank, dtype=np.float64)
    hit_mask  = data_rank < topk                              # bool [N_users]
    ndcg = np.where(hit_mask, 1.0 / np.log2(data_rank + 2), 0.0)
    hr   = hit_mask.astype(np.float64)
    return ndcg, hr
...
def record_csv(args, res_dict, path='log'):
    ...
    _suffix = ('_' + args.check_path) if getattr(args, 'check_path', '') else ''
    record_file = args.model_name + _suffix + '.csv'
