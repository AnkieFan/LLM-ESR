...
class Logger(object):
    ...
    def _create_logger(self):
        ...
        # get the current time string (append check_path so concurrent same-model
        # runs starting in the same second do not collide on the log directory)
        now_str = time.strftime("%m%d%H%M%S", time.localtime())
        if getattr(self.args, 'check_path', ''):
            now_str = now_str + '_' + self.args.check_path
