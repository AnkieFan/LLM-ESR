...
from utils.utils import unzip_data, unzip_data_with_user, concat_data
...
class Seq2SeqGeneratorAllUser(Generator):
    ...
    def make_trainloader(self):
        ...
        if self.args.aug:
            # all-prefix augmentation: keep original seqs + user mapping so self-distillation's
            # sim_users / sim-seq lookups still resolve to the correct ORIGINAL user.
            orig_seqs = unzip_data(self.train, aug=False)
            train_dataset, users = unzip_data_with_user(self.train, aug=True, aug_num=self.args.aug_seq_len)
            user_map = [u - 1 for u in users]   # 1-indexed user_id -> 0-based index (sim_users / orig_seqs)
            self.train_dataset = Seq2SeqDatasetAllUser(self.args, train_dataset, self.item_num, self.args.max_len,
                                                       self.args.train_neg, orig_seqs=orig_seqs, user_map=user_map)
        else:
            train_dataset = unzip_data(self.train, aug=False, aug_num=self.args.aug_seq_len)
