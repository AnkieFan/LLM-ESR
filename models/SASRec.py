...
class SASRec(BaseSeqModel):
    ...
    def __init__(self, user_num, item_num, device, args):
        ...
        self.ssm = getattr(args, "ssm", False)   # in-batch sampled-softmax loss (extension)
    ...
    def predict(self,
        ...
        if getattr(self, "usr_sem_train", False):   # D6 learnable-gamma user-semantic term
            logits = logits + self.usr_gamma * self._usr_sem_score(seq, item_indices, causal=False)
...
class SASRec_seq(SASRec):
    ...
    def forward(self, 
        ...
        if self.ssm:
            # in-batch sampled-softmax: each valid position's user-context scores its own positive
            # against ALL other valid positives in the batch (hundreds of negatives), with cross-entropy.
            feats_v = log_feats[indices]          # (M, C)
            pos_v = pos_embs[indices]             # (M, C)
            pos_ids = pos[indices]                # (M,)
            logits = feats_v @ pos_v.t()          # (M, M): row m vs candidate item n
            same = pos_ids.unsqueeze(0).eq(pos_ids.unsqueeze(1))      # mask false negatives (same item id)
            same.fill_diagonal_(False)
            logits = logits.masked_fill(same, float("-inf"))
            target = torch.arange(logits.shape[0], device=self.dev)  # true positive is on the diagonal
        ...
        if getattr(self, "usr_sem_train", False):   # D6 learnable-gamma term (causal centroid, no leak)
            pos_logits = pos_logits + self.usr_gamma * self._usr_sem_score(seq, pos, causal=True)
            neg_logits = neg_logits + self.usr_gamma * self._usr_sem_score(seq, neg, causal=True)
